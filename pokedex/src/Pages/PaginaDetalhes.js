...
const PaginaDetalhes = () => {
    // const [pokemons] = useRequestData([], `https://pokeapi.co/api/v2/pokemon/${params}`)
    ...
    const pokemonStats = pokemon && pokemon.stats?.map((s) => {
        return (
            <div key={s.id}>
    ...
    const pokemonTypes = pokemon && pokemon.types?.map((tipos) => {
        return (
            <div key={tipos.id}>
    ...
    const pokemonMoves = pokemon && pokemon.moves?.slice(0, 5).map((move) => {
        return (
            <div key={move.id}>
    ...
    //     if (states.pokedex.findIndex === pokemon) {
    ...
    return (
        <Div>
            <Header>
                <BotaoVoltar />
                <h1>{pokemon.name}</h1>
                {/* <BotaoAdicionar adicionarPokemon={addRemovePokemon} pokemon={pokemon} nome={"Adicionar/Remover da Pokedex"} /> */}
            </Header>
            <Main>
                <div>
